...
const BatchSummary = () => {
  ...
  const isEditable = isOperator && ["PENDING", "NOT_APPROVED"].includes((batch?.status || "").toUpperCase());
  ...
  const saveEdit = async (paymentId) => {
    try {
      setSaving(true);
      const res = await fetch(`http://localhost:8080/api/us4/batch/${batchesRef}/payment/${paymentId}`, {
        method: "PUT",
        headers: {
      ...
      if (res.status === 409) {
        alert("Edits are allowed only when batch status is Pending or Not Approved.");
        return;
      }
  ...
  return (
    <div className="container mt-4">
      ...
      {successMsg && (
        <div className="alert alert-success py-2" role="alert">{successMsg}</div>
      )}
